...
Kalman-рамка показывается только первые `prediction_display_frames` кадров после
потери. Затем рамка скрывается, но память цели и фоновый перезахват продолжают
работать до ручного сброса клавишей `r`.
